...
public class BasketFrame extends JFrame {
	...
	private void configureBodyPanel() {
		...
		RemoveTicketButton.addActionListener((ActionEvent e) -> {
			try {
				...
				query = "UPDATE seats SET seat_booked=false WHERE performance_id=" + performanceID + " AND seat_id="
						+ seatID;
				DBC.runQuery(query);
				DBC.close();
				...
				JOptionPane.showMessageDialog(null,
						"No row selected. Please select a row in the table to remove ticket.", "Error: No row selected",
						JOptionPane.ERROR_MESSAGE);
			}
		});
		bodyPanel.add(RemoveTicketButton);
	...
	public void loadTable() {
		String cols[] = new String[] { "Performance ID", "Seat No", "Seat Type", "Price", "Title", "Show Type", "Date",
				"Stage Time", "Duration", "Description" };
		model = (DefaultTableModel) table.getModel();
		DBConnector DBC = new DBConnector();
		DBC.connect();
		String query = "SELECT performances.performance_id, seat_id, standard_or_concession,  price, show_name, type , date, stage_time, duration, description  FROM basket JOIN performances ON basket.performance_id = performances.performance_id  JOIN shows ON performances.show_id = shows.show_id WHERE user_id="
				+ userID;
		...
		query = "SELECT MIN(date) FROM basket JOIN performances ON basket.performance_id=performances.performance_id  JOIN shows ON performances.show_id = shows.show_id WHERE user_id="
				+ userID;
		rs = DBC.runQuery(query);
		try {
			while (rs.next()) {
