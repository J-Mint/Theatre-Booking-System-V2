...
public class StagePerformanceFrame extends JFrame {
	...
	private void configureBodyPanel() {
		...
		String cols[] = new String[] { "Show ID", "Title", "Type", "Description", "Duration", "Language", "Live Music",
				"Performers" };
		...
		btnNewButton_1.addActionListener((ActionEvent e) -> {
			try {
				model.setRowCount(0);
				DBC.connect();
				String query1 = "SELECT * FROM shows WHERE show_name LIKE '%" + txtKeywords.getText()
						+ "%' OR type LIKE '%" + txtKeywords.getText() + "%' OR description LIKE '%"
						+ txtKeywords.getText() + "%' OR show_id LIKE '%" + txtKeywords.getText()
						+ "%' OR language LIKE '%" + txtKeywords.getText() + "%' OR duration LIKE '%"
						+ txtKeywords.getText() + "%' OR performers LIKE '%" + txtKeywords.getText() + "%'";
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				DBC.connect();
				String query = "CALL stagePerformance(" + textField_1.getText() + ", '" + textField.getText() + "', '"
						+ comboBox.getSelectedItem().toString() + "', " + textField_2.getText() + ", "
						+ textField_3.getText() + ")";
				DBC.runQuery(query);
				query = "SELECT performance_id FROM performances WHERE date='" + textField.getText()
						+ "' AND stage_time='" + comboBox.getSelectedItem().toString() + "'";
				ResultSet rs = DBC.runQuery(query);
				...
				for (int i = 1; i <= 200; i++) {
					...
					int seatBooked = 0;
					query = "INSERT INTO seats (seat_id, performance_id, seat_type, seat_booked) VALUES (" + seat_id
							+ ", " + performanceID + ", '" + seatType + "', '" + seatBooked + "')";
					DBC.runQuery(query);
				}
				DBC.close();
