...
public class RemoveShowFrame extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					RemoveShowFrame frame = new RemoveShowFrame();
	...
	public RemoveShowFrame() {
		...
		btnNewButton.addActionListener((ActionEvent e) -> {
			try {
				...
				int showID = Integer.parseInt((String) model.getValueAt(index, 0));
				int confirm = JOptionPane.showConfirmDialog(null, "You are about to remove show with ID: "+showID+". Is this correct?");
				if (confirm == 0) {
				DBC.connect();
				String query2 = "DELETE FROM shows WHERE show_id=" + showID;
				DBC.runQuery(query2);
				model.setRowCount(0);
				model.setColumnIdentifiers(cols);
